...
import { Header, Card, CardItem, Button, LoginForm, Spinner, WeakView, ProviderChooser, FirebaseScheduler } from './common';
import firebase from 'firebase';


class AppointApp extends Component {
  state = { servers: ['first', 'second'], servstring: 'load all', loaded: false, signedIn: null, cleanerlist: [], skeg: [], weeksting: '', months: [{days: 31, name: 'Jan'}, {days: 28, name: 'Feb'}, {days: 31, name: 'Mar'}, {days: 30, name: 'Apr'}, {days: 31, name: 'May'}, {days: 30, name: 'Jun'}, {days: 31, name: 'Jul'}, {days: 31, name: 'Aug'}, {days: 30, name: 'Sep'}, {days: 31, name: 'Oct'}, {days: 30, name: 'Nov'}, {days: 31, name: 'Dec'}], today: {}, thisday: {}, pro: '' };
  ...
  componentWillMount() {
    ...
    this.fbs = new FirebaseScheduler();
    this.fbs.loadAll(); 
    ...
    if(this.state.signedIn){
      return(
        <View style={{ marginBottom: 10 }}>
          ...
          <TouchableHighlight onPress={() => this.setState({ today: this.state.today }) }>
            {this.fbs.testListFilters()}
          </TouchableHighlight>
        </View>
      );
    }
